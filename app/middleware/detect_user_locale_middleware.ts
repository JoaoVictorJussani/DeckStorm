...
export default class DetectUserLocaleMiddleware {
  ...
  protected getRequestLocale(ctx: HttpContext) {
    ...
    if (queryLang) {
      const supported = i18nManager.getSupportedLocaleFor([queryLang])
      if (supported) {
        ctx.response.cookie('deckstorm_lang', supported)
        return supported
      }
    }

    // 2. Cookie
    const cookieLang = ctx.request.cookie('deckstorm_lang')
    if (cookieLang) {
      const supported = i18nManager.getSupportedLocaleFor([cookieLang])
      if (supported) {
        return supported
      }
    }
