...
const AuthController = () => import('#controllers/auth_controller') // Contrôleur pour l'authentification
const SocialAuthController = () => import('#controllers/social_auth_controller') // Contrôleur pour l'authentification OAuth
const PageController = () => import('#controllers/page_controller') // Contrôleur pour les pages
const DeckController = () => import('#controllers/deck_controller') // Contrôleur pour les decks
...
const CardController = () => import('#controllers/card_controller') // Contrôleur pour les cartes
...
router.get('/auth/:provider', [SocialAuthController, 'redirect']).as('auth.provider.redirect')
...
router.get('/deck/:id/like', [DeckController, 'like']).as('decks.like.get').use(middleware.auth())

router.post('/deck/:id/like', [DeckController, 'like']).as('decks.like').use(middleware.auth())
...
router.post('/decks', [DeckController, 'store']).as('decks.store').use(middleware.auth()) // Nécessite une authentification
...
router.get('/account/:id', [PageController, 'account']).as('account').use(middleware.auth())

// Route pour changer le mot de passe
router
  .post('/account/:id/change-password', '#controllers/auth_controller.changePassword')
  .use(middleware.auth())

// Route pour changer le nom d'utilisateur
router
  .post('/account/:id/change-username', '#controllers/auth_controller.changeUsername')
  .use(middleware.auth())
...
router
  .group(() => {
    ...
    router.post('/api/decks/:id/like', '#controllers/deck_controller.apiLike').as('api.decks.like')
...
router
  .post('/deck/:id/refuse-invite', async ({ params, auth, response, session }) => {
    const user = auth.user
    if (!user) return response.unauthorized('User not authenticated')
